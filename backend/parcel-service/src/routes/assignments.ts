    ...
    try {
      ...
      const riderRes = await fetch(`${RIDER_SERVICE_URL}/riders/assign-job`, {
        ...
        body: JSON.stringify({
          ...
          pickup_gps_lat:   parcel.pickup_lat ? parseFloat(parcel.pickup_lat) : null,
          pickup_gps_lng:   parcel.pickup_lng ? parseFloat(parcel.pickup_lng) : null,
          delivery_gps_lat: parcel.delivery_lat ? parseFloat(parcel.delivery_lat) : null,
          delivery_gps_lng: parcel.delivery_lng ? parseFloat(parcel.delivery_lng) : null,
          notes:            body.notes || '',
        }),
      });
